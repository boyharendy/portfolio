...
const ScrambledTitle: React.FC<{ phrases: string[] }> = ({ phrases }) => {
  ...
  return (
    <h1
      ref={elementRef}
      className="text-foreground text-3xl md:text-5xl lg:text-6xl font-bold tracking-wider text-center px-4 w-full"
      ...
      {characters.map((char, index) => (
        <span
          ...
          className={`absolute text-xs transition-colors duration-100 ${activeIndices.has(index)
            ? "text-green-500 text-base scale-125 z-10 font-bold animate-pulse"
            : "text-muted-foreground opacity-50 font-light"
            }`}
          style={{
            left: `${char.x}%`,
            top: `${char.y}%`,
            ...
            textShadow: activeIndices.has(index)
              ? '0 0 8px rgba(34,197,94,0.4), 0 0 12px rgba(34,197,94,0.2)'
              : 'none',
